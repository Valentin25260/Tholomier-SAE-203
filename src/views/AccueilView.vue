<template>
  <div class="relative">
    <img src="../../public/accueil.png" class="w-full h-auto" />

    <svg
      class="absolute top-0 left-0 w-[58.2%] h-auto"
      width="691"
      height="428"
      viewBox="0 0 691 518"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      preserveAspectRatio="none"
    >
      <path
        d="M0 0H435.172L691 518H0V0Z"
        fill="#FF9C7D"
        fill-opacity="0.80"
      ></path>
    </svg>

    <div class="absolute top-[15px] left-[2%] w-[62.2%]">
      <p class="w-[375px] h-15 text-[32px] text-left">
        <span
          class="
            w-[405px]
            h-24
            text-[32px]
            sm:text-[80px]
            text-left text-black
            font-bebas
          "
          >Espi </span
        ><span
          class="
            w-[405px]
            h-24
            text-[32px]
            sm:text-[80px]
            text-left text-blue-500
            font-bebas
          "
          >Festi</span
        >
      </p>

      <p class="w-[165px] h-[41px] text-[16px] text-left text-white font-bebas">
        Le premier festival de france sur une plage.
      </p>
    </div>

    <p class="font-bebas text-2xl mx-14 text-center mt-4">
      <span class="text-blue-500">Retrouvez nous le</span>
      <span class="text-[#FF9C7D]"> 1,2 et 3 juillet</span>
      <span class="text-blue-500"> au festival Espi festi pour notre </span>
      <span class="text-[#FF9C7D]">première édition.</span>
    </p>

    <img src="../../public/carré.png" class="w-full mt-8 h-auto" />

    <div class="flex flex-row flex-wrap mt-8 w-full">
      <img src="../../public/image1.webp" class="w-1/2 h-auto" />
      <img src="../../public/image2.webp" class="w-1/2 h-auto" />
      <img src="../../public/image3.webp" class="w-full h-auto" />
    </div>
  </div>
</template>




